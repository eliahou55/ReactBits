...
const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    let resizeRaf = 0;
    const scheduleResize = () => { if (resizeRaf) cancelAnimationFrame(resizeRaf); resizeRaf = requestAnimationFrame(setSizeNow); };
    setSizeNow();
    ...
    const adjustDprIfNeeded = now => {
      ...
      if (avgFps < 50) next = clamp(currentDprRef.current * 0.85, dprFloor, base);
      else if (avgFps > 58 && currentDprRef.current < base) next = clamp(currentDprRef.current * 1.1, dprFloor, base);
      if (Math.abs(next - currentDprRef.current) > 0.01 && now - lastDprChangeRef > dprChangeCooldown) {
        currentDprRef.current = next; lastDprChangeRef = now; setSizeNow();
      }
